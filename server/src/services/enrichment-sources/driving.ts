...
const DRIVING_DATA: Record<string, DrivingRequirement> = {
  ...
  HT: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Road conditions are extremely poor and security situation is dangerous. Self-driving is not recommended.' },
  DO: { driving_side: 'right', idp_required: true, min_age: 18 },
  PR: { driving_side: 'right', idp_required: false, min_age: 16 },
  BB: { driving_side: 'left', idp_required: false, min_age: 18, note: 'A local Barbados driving permit is required alongside your foreign license, available from police stations.' },
  TT: { driving_side: 'left', idp_required: true, min_age: 17 },
  BS: { driving_side: 'left', idp_required: true, min_age: 17 },
  BM: { driving_side: 'left', idp_required: false, min_age: 18, note: 'Rental cars are not available in Bermuda. Visitors may hire mopeds or taxis only.' },
  LC: { driving_side: 'left', idp_required: false, min_age: 18, note: 'A temporary St. Lucia driving permit (obtainable on arrival) is required alongside your foreign license.' },
  VC: { driving_side: 'left', idp_required: false, min_age: 17 },
  AG: { driving_side: 'left', idp_required: false, min_age: 17, note: 'A local Antigua driving permit is required, available from police stations.' },
  ...
  VE: { driving_side: 'right', idp_required: true, min_age: 18, note: 'The security situation is dangerous. Self-driving outside major cities is not recommended.' },
  EC: { driving_side: 'right', idp_required: true, min_age: 18 },
  BO: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Road conditions are hazardous in highland areas. The Death Road (North Yungas Road) is extremely dangerous.' },
  ...
  LB: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Road conditions and security situation vary. Exercise caution especially near border regions.' },
  IQ: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Most governments advise against all travel to Iraq. Self-driving is strongly discouraged.' },
  IR: { driving_side: 'right', idp_required: true, min_age: 18 },
  YE: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Most governments advise against all travel to Yemen due to ongoing conflict.' },
  ...
  PK: { driving_side: 'left', idp_required: true, min_age: 18, note: 'Road conditions are poor in many areas. Traffic laws are loosely enforced.' },
  BD: { driving_side: 'left', idp_required: true, min_age: 18, note: 'Traffic conditions in Dhaka are extremely congested. Self-driving is not recommended for visitors.' },
  LK: { driving_side: 'left', idp_required: true, min_age: 18 },
  NP: { driving_side: 'left', idp_required: true, min_age: 18, note: 'Mountain roads can be extremely narrow and dangerous, especially in monsoon season.' },
  BT: { driving_side: 'left', idp_required: true, min_age: 18 },
  MV: { driving_side: 'left', idp_required: true, min_age: 18, note: 'Most visitors travel between islands by boat. Road networks are very limited.' },
  ...
  CN: { driving_side: 'right', idp_required: false, min_age: 18, note: 'Foreign licenses are not valid in China. Tourists cannot legally drive. A Chinese license is required.' },
  ...
  KP: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Tourism is extremely restricted. Self-driving by foreign nationals is not permitted.' },
  MN: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Most roads outside Ulaanbaatar are unpaved. Off-road driving experience is highly recommended.' },
  VN: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Traffic conditions are chaotic. Self-driving is challenging; guided tours or rental scooters are more common.' },
  ...
  MM: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Despite historical left-hand traffic patterns, Myanmar switched to right-hand traffic in 1970. Many vehicles remain right-hand drive.' },
  MY: { driving_side: 'left', idp_required: true, min_age: 17 },
  SG: { driving_side: 'left', idp_required: true, min_age: 18 },
  ID: { driving_side: 'left', idp_required: true, min_age: 17, note: 'Traffic conditions in Jakarta and Bali are extremely congested. Scooter rentals are very common in Bali.' },
  ...
  CK: { driving_side: 'left', idp_required: false, min_age: 21, note: 'A Cook Islands driving license is required. It can be obtained on arrival from the police.' },
  ...
  LY: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Most governments advise against all travel to Libya due to ongoing conflict.' },

  // Sub-Saharan Africa
  ZA: { driving_side: 'left', idp_required: true, min_age: 18 },
  NG: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Road conditions are poor and traffic laws are loosely enforced. Carjacking is a risk on major highways.' },
  ...
  SS: { driving_side: 'right', idp_required: true, min_age: 18, note: 'Most governments advise against all travel to South Sudan due to ongoing instability.' },
